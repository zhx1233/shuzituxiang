...
class SaltAndPepperItem(MyItem):
    def __init__(self, parent=None):
        super().__init__('椒盐噪声', parent=parent)
        self._salt_threshold = 100  # 食盐噪声阈值（低于此值的像素可能被替换为白色）
        self._pepper_threshold = 200  # 胡椒噪声阈值（高于此值的像素可能被替换为黑色）
        self._noise_ratio = 0.1  # 噪声比例，控制多少像素会被添加噪声
    ...
    def __call__(self, img):
        ...
        noise_pixels = int(total_pixels * self._noise_ratio)
        ...
        if len(img.shape) == 2:  # 灰度图像
            for _ in range(noise_pixels):
                ...
                pixel = img[i, j]
                if pixel < self._salt_threshold:
                    output[i, j] = 255  # 食盐噪声
                elif pixel > self._pepper_threshold:
                    output[i, j] = 0  # 胡椒噪声
        else:  # 彩色图像
            for _ in range(noise_pixels):
                i = np.random.randint(0, img.shape[0])
                j = np.random.randint(0, img.shape[1])
                for c in range(img.shape[2]):  # 对每个通道处理
                    pixel = img[i, j, c]
                    if pixel < self._salt_threshold:
                        output[i, j, c] = 255
                    elif pixel > self._pepper_threshold:
