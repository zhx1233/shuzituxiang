...
class HoughLineTableWidget(TableWidget):
    def __init__(self, parent=None):
        ...
        self.thresh_spinBox = QSpinBox()
        self.thresh_spinBox.setMinimum(0)
        self.thresh_spinBox.setMaximum(1000)  # 设置交点阈值的最大值为 255
        self.thresh_spinBox.setSingleStep(1)
        self.thresh_spinBox.setObjectName('thresh')

        self.min_length_spinBox = QSpinBox()
        self.min_length_spinBox.setMinimum(0)
        self.min_length_spinBox.setMaximum(1000)  # 设置交点阈值的最大值为 255
        ...
        self.max_gap_spinbox.setMaximum(1000)  # 设置交点阈值的最大值为 255
...
class SaltAndPepperTableWidget(TableWidget):
    def __init__(self, parent=None):
        super(SaltAndPepperTableWidget).__init__(parent=parent)

        # 食盐噪声阈值控件
        self.salt_threshold_spin = QSpinBox()
        self.salt_threshold_spin.setObjectName('salt_threshold')
        self.salt_threshold_spin.setRange(0, 255)
        self.salt_threshold_spin.setValue(100)
        self.salt_threshold_spin.setSuffix(' (越低添加越多白色噪声)')
        ...
        self.noise_ratio_spin.setRange(0.0, 1.0)
        self.noise_ratio_spin.setValue(0.1)
        self.noise_ratio_spin.setSingleStep(0.01)
